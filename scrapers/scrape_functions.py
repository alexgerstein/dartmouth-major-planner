...
def remove_deleted_offerings(timetable_globals):
    ...
    for offering in deleted_offerings:
        ...
        if offering.term is not None:
            if not offering.term.in_range(oldest_term, latest_lock_term):
                ...
                if other_time:
                    ...
                    if str(offering.hour) != str(other_time.hour):
                        print "EMAIL 3"
                        # emails.swapped_course_times(emailed_users, offering, other_time)
                    print_alert("SWAPPED: " + repr(offering.term) + " " + repr(offering) + " at " + repr(offering.hour) + "with " + repr(other_time.hour))
                else:
                    print "EMAIL 4"
                    # emails.deleted_offering_notification(emailed_users, offering, offering.term, offering.hour)
...
def add_offerings(course, terms_offered, hours_offered, distribs, course_desc, lock_term_start, lock_term_end):
    ...
    for term in terms_offered:
        ...
        for hour in hours_offered:
            ...
            for distrib in distribs:
                if distrib not in o1.distributives:
                    o1.distributives.append(distrib)
                    db.session.commit()
